...
public class InstallerMeta {
  ...
  public void load() throws IOException, JSONException {
    ...
    json.getJSONArray("game_versions")
        .toList()
        .forEach(element -> gameVersions.add(element.toString()));
  ...
  public static class Edition {
    ...
    public Edition(JSONObject jsonObject) {
      ...
      for (int i = 0; i < jsonObject.getJSONArray("compatible_versions").toList().size(); i++) {
        compatibleVersions.add(
            jsonObject.getJSONArray("compatible_versions").toList().get(i).toString());
      }
    }
  }
}
