...
public class Installer {
  ...
  public void start() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          null,
          "The installer was unable to fetch fabric version info from the server, please check your"
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          null,
          "The installer was unable to fetch metadata from the server, please check your internet"
      ...
      JOptionPane.showMessageDialog(
          null,
          "Installer metadata parsing failed, please contact Justsnoopy30! \nError: " + e,
    ...
    frame.setIconImage(
        new ImageIcon(
                Objects.requireNonNull(
                    Utils.class.getClassLoader().getResource("iris_profile_icon.png")))
            .getImage());
    ...
    editionDropdown.addItemListener(
        e -> {
    ...
    versionDropdown.addItemListener(
        e -> {
          if (e.getStateChange() == ItemEvent.SELECTED) {
    ...
    installDirectoryPicker.addActionListener(
        e -> {
    ...
    installAsModCheckbox.addActionListener(
        e -> {
    ...
    button.addActionListener(
        action -> {
          if (!EDITIONS.stream()
              .filter(edition -> edition.name.equals(selectedEditionName))
              .findFirst()
              .get()
              .compatibleVersions
            ...
            JOptionPane.showMessageDialog(
                frame,
                "The selected edition is not compatible with the chosen game version.",
          ...
          if (!installAsMod) {
            Reference.metaServerUrl =
                "https://raw.githubusercontent.com/IrisShaders/Iris-Installer-Maven/master/";
          ...
          try {
            URL loaderVersionUrl =
                new URL(
                    "https://raw.githubusercontent.com/IrisShaders/Iris-Installer-Maven/master/latest-loader");
            String loaderVersion =
                installAsMod
                    ? Main.LOADER_META.getLatestVersion(false).getVersion()
                    : Utils.readTextFile(loaderVersionUrl);
            VanillaLauncherIntegration.installToLauncher(
                getVanillaGameDir(),
                getInstallDir(),
                installAsMod ? "Fabric Loader " + selectedVersion : selectedEditionDisplayName,
                selectedVersion,
                loaderName,
                loaderVersion,
                installAsMod
                    ? VanillaLauncherIntegration.Icon.FABRIC
                    : VanillaLauncherIntegration.Icon.IRIS);
          } catch (IOException e) {
            System.out.println("Failed to install version and profile to vanilla launcher!");
            e.printStackTrace();
            JOptionPane.showMessageDialog(
                frame,
                "Failed to install to vanilla launcher, please contact Justsnoopy30! \nError: " + e,
              ...
              event -> {
                if ("progress".equals(event.getPropertyName())) {
                  ...
                  try {
                    ...
                    String msg =
                        String.format(
                            "An error occurred while attempting to download the required files,"
                                + " please check your internet connection and try again! \n"
                                + "Error: %s",
                            e.getCause().toString());
                    JOptionPane.showMessageDialog(
                        frame, msg, "Download Failed!", JOptionPane.ERROR_MESSAGE, null);
                    readyAll();
                  ...
                  File modsFolder =
                      getInstallDir().resolve(installAsMod ? "mods" : "iris-reserved").toFile();
                  ...
                  if (modsFolderContents != null) {
                    ...
                    if (installAsMod
                      ...
                      int result =
                          JOptionPane.showConfirmDialog(
                              frame,
                              "An existing mods folder was found in the selected game directory. Do"
                                  + " you want to update/install iris?",
                    ...
                    if (!cancelled) {
                      ...
                      for (File mod : modsFolderContents) {
                        if (mod.getName().toLowerCase().contains("optifine")
                            || mod.getName().toLowerCase().contains("optifabric")) {
                          if (!shownOptifineDialog) {
                            int result =
                                JOptionPane.showOptionDialog(
                                    frame,
                                    "Optifine was found in your mods folder, but Optifine is"
                                        + " incompatible with Iris. Do you want to remove it, or"
                      ...
                      if (failedToRemoveOptifine) {
                        ...
                        JOptionPane.showMessageDialog(
                            frame,
                            "Failed to remove optifine from your mods folder, please make sure your"
                                + " game is closed and try again!",
                            "Failed to remove optifine",
                            JOptionPane.ERROR_MESSAGE);
                        cancelled = true;
                      }
                    }
                    ...
                    if (!cancelled) {
                      ...
                      for (File mod : modsFolderContents) {
                        if (mod.getName().toLowerCase().contains("iris")
                            || mod.getName().toLowerCase().contains("sodium-fabric")) {
                      ...
                      if (failedToRemoveIrisOrSodium) {
                        System.out.println(
                            "Failed to remove Iris or Sodium from mods folder to update them!");
                        JOptionPane.showMessageDialog(
                            frame,
                            "Failed to remove iris and sodium from your mods folder to update them,"
                                + " please make sure your game is closed and try again!",
                            "Failed to prepare mods for update",
                            JOptionPane.ERROR_MESSAGE);
                        cancelled = true;
                      }
                    }
                  ...
                  if (installSuccess) {
                    ...
                    JOptionPane.showMessageDialog(
                        frame,
                        "Failed to install to mods folder, please make sure your game is closed and"
                            + " try again!",
                        "Installation Failed!",
                        JOptionPane.ERROR_MESSAGE);
                  }
  ...
  public String getStorageDirectoryName() {
    String os = System.getProperty("os.name").toLowerCase();
    if (os.contains("mac")) return "iris-installer";
    else return ".iris-installer";
  }
  ...
  public Path getVanillaGameDir() {
    ...
    return os.contains("mac")
        ? getAppDataDirectory().resolve("minecraft")
        : getAppDataDirectory().resolve(".minecraft");
  }
